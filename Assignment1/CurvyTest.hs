...
testStrings = ["c = (4.0,3)", -- Testing Curve, indent and tests the number can recognise doubles with and without decimals
               ...
               "c = (4,3) ++ (5,3) rot (4,3) 4", -- Define and rotate a curve (checking presedence)
...
test = foldl (\ x y -> x && y) True list 
      where list = (runTests testStrings resultStrings) ++ (runTests failStrings [False, False, False, False, False]) 
